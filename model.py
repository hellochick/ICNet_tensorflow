...
class ICNet_BN(Network):
    def __init__(self, images, cfg):
        ...
        self.output = self.get_output_node()
        self.layers["logits"] = tf.image.resize_bilinear(self.layers['conv6_cls'], size=tf.shape(self.images)[1:3], align_corners=True)
        self.layers["output"] = self.output
    ...
    def get_output_node(self):
        ...
        logits_up = tf.image.resize_bilinear(logits, size=tf.shape(self.images)[1:3], align_corners=True)
